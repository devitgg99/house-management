...
export function EditUtilityDialog({
  ...
  useEffect(() => {
    if (isOpen && utility) {
      ...
      const secureUrl = ensureHttps(utility.meterImageUrl);
      setMeterImageUrl(secureUrl || null);
      setMeterImagePreview(secureUrl || null);
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (result.success && result.url) {
        ...
        toast.success("Meter photo updated");
      } else {
        ...
        setMeterImagePreview(meterImageUrl);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!session?.user?.token || !utility) {
      browserLogger.warn("Utility", "Authentication or utility record missing when editing utility", {
    ...
    try {
      ...
      if (result.success) {
        browserLogger.success("Utility", `Utility updated for ${utility.roomName}: ${newWater - oldWater} units`, {
          payload,
          result,
        });
        ...
        const targetMonth = utility.month;
        onClose();
        onSuccess?.(targetMonth);
      } else {
        browserLogger.error("Utility", "Failed to update utility reading", {
          error: result.error,
          payload,
          result,
        });
      ...
      browserLogger.error("Utility", "Network or server exception while updating utility", {
        error,
        utilityId: utility.utilityId,
        formData,
      });
